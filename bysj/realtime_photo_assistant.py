...
def noise_score(gray):
    ...
    # 经验阈值：<8优秀，>20较差
    score = max(0, 100 - (noise-8)*5)
    status = '优秀' if noise < 8 else ('一般' if noise < 15 else '较多')
    return int(score), status, noise
...
def composition_score(img, model):
    ...
    for box, cls in zip(pred.boxes.xyxy.cpu().numpy(), pred.boxes.cls.cpu().numpy()):
        ...
        result = f"{class_name} {'在三分线上' if on_third else ''}{'在黄金分割点' if on_golden else ''}，面积占比{area:.2f}，边界惩罚{penalty}"
...
def main():
    ...
    if uploaded_file:
        ...
        # 3. 噪点
        noi_score, noi_status, noise = noise_score(gray)
        # 4. 构图
        model = load_yolo()
        ...
        st.write(f'噪点：{noi_score}/100（{noi_status}，噪点估计{noise:.2f}）')
